	...
	public class LogParser : ILogParser
	{
		...
		public PagedResult<LogMessage> Search(int pageNumber = 1, int pageSize = 100, string filterExpression = null, SortOrder sort = SortOrder.Descending)
		{
			...
			if (filterExpression == null)
			{
				...
				var noFilter = new PagedResult<LogMessage>(totalRecords, pageNumber, pageSize)
				{
					Items = logMessages,
					ErrorCount = _logItems.Count(log => log.Level == LogEventLevel.Fatal || log.Level == LogEventLevel.Error || log.Exception != null),
					LogLevelCounts = new LogLevelCounts
					{
						Verbose = _logItems.Count(log => log.Level == LogEventLevel.Verbose),
						Information = _logItems.Count(log => log.Level == LogEventLevel.Information),
						Debug = _logItems.Count(log => log.Level == LogEventLevel.Debug),
			...
			var rez = new PagedResult<LogMessage>(filteredTotal, pageNumber, pageSize)
			{
				Items = logItems,
				ErrorCount = filteredLogs.Count(log => log.Level == LogEventLevel.Fatal || log.Level == LogEventLevel.Error || log.Exception != null),
				LogLevelCounts = new LogLevelCounts
				{
					Verbose = filteredLogs.Count(log => log.Level == LogEventLevel.Verbose),
					Information = filteredLogs.Count(log => log.Level == LogEventLevel.Information),
					Debug = filteredLogs.Count(log => log.Level == LogEventLevel.Debug),
					Warning = filteredLogs.Count(log => log.Level == LogEventLevel.Warning),
					Error = filteredLogs.Count(log => log.Level == LogEventLevel.Error),
					Fatal = filteredLogs.Count(log => log.Level == LogEventLevel.Fatal)
				},
				LogTemplates = filteredLogs
					.GroupBy(log => log.MessageTemplate.Text)
					.Select(x => new LogTemplate
					{
						MessageTemplate = x.Key,
